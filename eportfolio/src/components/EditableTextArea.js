...
export function EditableTextArea(props) {
    ...
    return (
        <p>{text} {props.isEditable && <button className="btn edit-icon-button" onClick={()=>setIsEditing(true)}>
...
export function EditableTextField(props) {
    ...
    return (
        <span>{text}  {props.isEditable && <button className="btn edit-icon-button" onClick={()=>setIsEditing(true)}>
            <FontAwesomeIcon icon={faEdit} /></button> }
        </span>
    )
}